...
export function RegistrarPago({ preselectedClienteId, onSaved, onCancel }: RegistrarPagoProps) {
  ...
  return <div className="p-6 max-w-2xl"><div className="flex items-center gap-3 mb-6"><button onClick={onCancel} className="text-[var(--text-secondary)] hover:text-[var(--text-primary)] transition-colors cursor-pointer"><IcoBack /></button><h1 className="text-xl font-semibold text-[var(--text-primary)]">Registrar pago</h1></div><div className="space-y-5"><div className="grid grid-cols-1 sm:grid-cols-2 gap-4"><FieldSelect label="Cliente" value={clienteId} onChange={event => { setClienteId(event.target.value); setDistrib({}); setDeudasActivas([]) }}><option value="">Seleccionar cliente…</option>{clientes.map(cliente => <option key={cliente.id} value={cliente.id}>{cliente.nombre}</option>)}</FieldSelect><FieldInput label="Fecha del pago" type="date" value={fecha} onChange={event => setFecha(event.target.value)} /></div><div className="grid grid-cols-1 sm:grid-cols-2 gap-4"><FieldInput label="Monto recibido (COP)" type="number" min="0" step="500" value={monto} onChange={event => setMonto(event.target.value)} placeholder="Ej: 15000" /><FieldSelect label="Método" value={metodo} onChange={event => setMetodo(event.target.value as 'efectivo' | 'transferencia')}><option value="efectivo">Efectivo</option><option value="transferencia">Transferencia</option></FieldSelect></div>
    {clienteId && <div><div className="flex items-center justify-between mb-2"><label className="text-[10px] font-semibold text-[var(--text-secondary)] uppercase tracking-widest">Distribuir a deudas</label>{montoNum > 0 && <span className={`text-xs font-mono font-semibold ${restante < 0 ? 'text-[var(--accent-red)]' : restante === 0 ? 'text-[var(--accent-green-light)]' : 'text-[var(--accent-yellow)]'}`}>Restante: {fmt(restante)}</span>}</div>
      ...
      {errors.length > 0 && <div className="mt-2 space-y-1">{errors.map((error, index) => <p key={index} className="text-xs text-[var(--accent-red)]">⚠ {error}</p>)}</div>}</div>}
    {submitError && <p className="text-xs text-[var(--accent-red)]">⚠ {submitError}</p>}
...
function DeudasPendientesDelCliente({ clienteId, distrib, onDistribChange, montoNum, onDeudasLoaded }: DeudasPendientesDelClienteProps) {
  ...
  if (loading) return <Card className="p-4 text-sm text-[var(--text-secondary)] text-center">Cargando deudas…</Card>
  if (error) return <Card className="p-4 text-sm text-[var(--accent-red)] text-center">⚠ {error}</Card>
  if (deudas.length === 0) return <Card className="p-4 text-sm text-[var(--text-muted)] text-center">Este cliente no tiene deudas pendientes.</Card>

  const totalDistrib = Object.values(distrib).reduce((sum, value) => sum + (parseFloat(value) || 0), 0)

  return <Card className="overflow-hidden divide-y divide-[var(--bg-elevated)]">
    {deudas.map(deuda => <div key={deuda.id} className="p-3 flex items-center gap-3"><div className="flex-1 min-w-0"><div className="text-sm text-[var(--text-primary)]">{fmtDate(deuda.fecha_fiado.slice(0, 10))}</div></div><div className="shrink-0 text-right mr-2"><div className="text-[10px] text-[var(--text-secondary)]">Saldo</div><div className="text-sm font-mono font-semibold text-[var(--accent-red)]">{fmt(deuda.saldo)}</div></div><div className="w-28 shrink-0"><input type="number" min="0" max={deuda.saldo} step="500" placeholder="0" value={distrib[deuda.id] ?? ''} onChange={event => onDistribChange(previous => ({ ...previous, [deuda.id]: event.target.value }))} className="w-full px-2 py-1.5 bg-[var(--bg-elevated)] border border-[var(--border)] rounded text-sm font-mono text-[var(--text-primary)] focus:outline-none focus:border-[var(--accent-blue)] text-right" /></div></div>)}
    {montoNum > 0 && <div className="px-3 py-2.5 flex justify-between bg-[var(--bg-app)]"><span className="text-xs text-[var(--text-secondary)]">Total distribuido</span><span className="text-sm font-mono font-semibold text-[var(--text-primary)]">{fmt(totalDistrib)} <span className="text-[var(--text-secondary)] font-normal">de {fmt(montoNum)}</span></span></div>}
  </Card>
}
