...
export function Dashboard({ onNavigate }: DashboardProps) {
  ...
  return (
    <div className="p-6 max-w-3xl">
      ...
      {loading ? <div className="text-sm text-[#7d8590]">Cargando…</div> : <>
              ...
              return (
                <div key={deuda.id} className="px-4 py-3 flex items-center gap-3 hover:bg-[#21262d]/40 cursor-pointer transition-colors" onClick={() => onNavigate('cliente-detalle', deuda.cliente_id)}>
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="text-xs text-[#7d8590] mt-0.5">{fmtDate(deuda.fecha_fiado.slice(0, 10))}</div>
                  </div>
                  <EstadoBadge estado={deuda.estado} />
                  <div className="text-sm font-mono font-semibold text-[#e6edf3] shrink-0">{fmt(deuda.saldo)}</div>
