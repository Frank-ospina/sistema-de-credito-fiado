...
export function NuevaDeuda({ preselectedClienteId, onSaved, onCancel }: NuevaDeudaProps) {
  ...
  return <div className="p-6 max-w-2xl"><div className="flex items-center gap-3 mb-6"><button onClick={onCancel} className="text-[var(--text-secondary)] hover:text-[var(--text-primary)] transition-colors cursor-pointer"><IcoBack /></button><h1 className="text-xl font-semibold text-[var(--text-primary)]">Nueva deuda (fiado)</h1></div><div className="space-y-5">
    ...
    <div><div className="flex items-center justify-between mb-2"><label className="text-[10px] font-semibold text-[var(--text-secondary)] uppercase tracking-widest">Productos</label><Btn variant="ghost" size="sm" onClick={() => setItems(previous => [...previous, { productoId: '', cantidad: 1 }])}><IcoPlus />Agregar línea</Btn></div><Card className="overflow-hidden"><div className="grid grid-cols-[1fr_72px_100px_32px] border-b border-[var(--border)] bg-[var(--bg-surface)]">{['Producto', 'Cant.', 'Subtotal', ''].map(header => <div key={header} className="px-3 py-2 text-[10px] font-semibold text-[var(--text-secondary)] uppercase tracking-widest">{header}</div>)}</div>{itemsRich.map((item, index) => <div key={index} className="grid grid-cols-[1fr_72px_100px_32px] border-t border-[var(--bg-elevated)]"><select className="px-3 py-2.5 bg-transparent text-sm text-[var(--text-primary)] focus:outline-none border-r border-[var(--bg-elevated)]" value={item.productoId} onChange={event => setItems(previous => previous.map((value, itemIndex) => itemIndex === index ? { ...value, productoId: event.target.value } : value))}><option value="">Seleccionar…</option>{productos.map(producto => <option key={producto.id} value={producto.id}>{producto.nombre} — {fmt(producto.precio_actual)}</option>)}</select><input type="number" min="1" className="px-3 py-2.5 bg-transparent text-sm text-[var(--text-primary)] font-mono focus:outline-none text-center border-r border-[var(--bg-elevated)]" value={item.cantidad} onChange={event => setItems(previous => previous.map((value, itemIndex) => itemIndex === index ? { ...value, cantidad: Math.max(1, parseInt(event.target.value) || 1) } : value))} /><div className="px-3 flex items-center border-r border-[var(--bg-elevated)]"><span className="text-sm font-mono text-[var(--text-primary)]">{fmt(item.subtotal)}</span></div><div className="flex items-center justify-center">{items.length > 1 && <button onClick={() => setItems(previous => previous.filter((_, itemIndex) => itemIndex !== index))} className="text-[var(--text-muted)] hover:text-[var(--accent-red)] cursor-pointer transition-colors p-1"><IcoTrash /></button>}</div></div>)}<div className="flex items-center justify-end gap-3 px-3 py-2.5 border-t border-[var(--border)] bg-[var(--bg-app)]"><span className="text-xs text-[var(--text-secondary)]">Total a fiar</span><span className="text-base font-mono font-semibold text-[var(--text-primary)]">{fmt(total)}</span></div></Card></div>
